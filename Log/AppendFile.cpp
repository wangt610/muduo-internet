...
#include <cstring>
    ...
    {
        return fwrite(info, 1, len, fp_);
    }

AppendFile::AppendFile(const std::string &filename):fp_{nullptr},buffer_(new char[FILEBUFFSIZE]),wittenBytes_(0){
   // cout<<filename<<endl;
    fp_ = fopen(filename.c_str(), "a");
    assert(fp_!=nullptr);
    setvbuf(fp_, buffer_.get(), _IOFBF,FILEBUFFSIZE);
}
